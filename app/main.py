...
from app.utils.scraper import fetch_games_from_web, update_scores_from_web, fetch_betting_odds
...
async def serve_react(full_path: str):
    ...
    """
    api_prefixes = ["users", "betting-leagues", "games", "bets"]  # ✅ Add all API prefixes
    ...
    return JSONResponse(status_code=404, content={"error": "React build not found. Run `npm run build` in `frontend/`"})



def fetch_and_store_games(db: Session, target_date: str = None):
    logger.info(f"🔄 Fetching {'all games' if not target_date else f'games for {target_date}'} from web")
    ...
    for game in games:
        db_game = db.query(Game).filter(
            (Game.team1 == game.team1) & (Game.team2 == game.team2) & (Game.match_time == game.match_time)
        ).first()
        if not db_game:
            db.add(game)
            added_games += 1
...
def startup_tasks():
    ...
    thread = threading.Thread(target=scheduled_game_updates, args=(next(get_db()),), daemon=True)
    thread.start()
