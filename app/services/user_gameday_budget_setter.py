...
def get_gamday_data(db: Session):
    ...
    ).group_by(func.date(Game.match_time)).order_by(func.date(Game.match_time)).all()
    gameday_data = []
    for row in results:
        ...
        first_game = (
            ...
            .filter(
                ...
                Game.match_time == row.first_game_time  # ✅ Get the first game of that day
            )
            .first()
        )
        gameday_data.append({
            ...
            "stage": first_game.stage if first_game else "Unknown"  # ✅ Assign stage
        })
    return gameday_data
...
def set_gameday_budget(db: Session):
    ...
    for gameday in gameday_data:
        ...
        stage = gameday["stage"]
        gameday_budget = num_of_games * settings.STAGE_TO_GAMEDAY_BUDGET_KEY_MAPPING.get(stage, 0)
